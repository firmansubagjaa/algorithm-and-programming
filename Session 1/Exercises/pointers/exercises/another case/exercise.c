...
void doubleQuote(int *ptr, int length) {
   for (int i = 0; i < length; i++) {
      *(ptr + i) *= 2;
   }
}
...
/*
   Menjalankan program
      melakukan pengetesan program sesudah implementasikan
      algoritma ke program dengan menguji program tersebut
      meliputi dengan menyiapkan sebuah array of numbers
      dengan memiliki elemen
         [10, 20, 30, 40, 50]
   
   Perbaikan kesalahan
      sesudah pengujian, program tersebut sudah
      berjalan sesuai harapan sehingga tidak butuh
      melakukan debugging
   
   Dokumentasi
      Program ini dirancang untuk melakukan double quote
*/
